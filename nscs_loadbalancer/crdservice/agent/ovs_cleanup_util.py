...
from nscs_loadbalancer.crdservice.agent.common import config as agent_config
from nscs_loadbalancer.crdservice.agent import l3_agent
...
from nscs_loadbalancer.crdservice.agent.linux import ovs_lib
from nscs.crdservice.common import config
    ...
    opts = [
        cfg.BoolOpt('ovs_all_ports',
                    ...
                    help=_('True to delete all ports on all the OpenvSwitch '
                           ...
                           'Crd on integration and external network '
                           'bridges.'))
    ]
...
def collect_neutron_ports(bridges, root_helper):
    """Collect ports created by Crd from OVS."""
    ports = []
...
def delete_neutron_ports(ports, root_helper):
    """Delete non-internal ports created by Crd
...
def main():
    ...
    The utility cleans up the integration bridges used by Crd.
    """
    ...
    # Collect existing ports created by Crd on configuration bridges.
    # After deleting ports from OVS bridges, we cannot determine which
    # ports were created by Crd, so port information is collected now.
    ...
    # Remove remaining ports created by Crd (usually veth pair)
