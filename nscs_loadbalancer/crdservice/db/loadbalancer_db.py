...
from nscs_loadbalancer.crdservice.extensions.loadbalancer import LoadBalancerPluginBase
from nscs.crdservice import manager
from nscs.crdservice.openstack.common.db import exception
...
from nscs_loadbalancer.crdservice import constants as lb_const
from cns.crdservice.db import network as network_db
from nscs_loadbalancer.crdservice.drivers.haproxy import cfg as hacfg
...
class Vip(model_base.BASEV2, HasId, HasTenant,
          HasStatusDescription):
    """Represents a v2 neutron loadbalancer vip."""

    name = sa.Column(sa.String(255))
    description = sa.Column(sa.String(255))
    port_id = sa.Column(sa.String(36), sa.ForeignKey('cns_ports.id'))
    ...
    port = orm.relationship(network_db.CrdPort, backref="vips", lazy='joined')


class Member(model_base.BASEV2, HasId, HasTenant,
             HasStatusDescription):
...
class Pool(model_base.BASEV2, HasId, HasTenant,
           HasStatusDescription):
...
class HealthMonitor(model_base.BASEV2, HasId, HasTenant):
...
class LoadBalancerPluginDb(LoadBalancerPluginBase,
                           base_db.CrdDbPluginV2):
    """Wraps loadbalancer with SQLAlchemy models.

    A class that wraps the implementation of the Crd loadbalancer
    ...
    def _make_vip_dict(self, vip, fields=None):
        if vip.port_id:
            subnet_id = vip.port.subnet_id
            ip_address = vip.port.ip_address
        else:
            subnet_id = None
            ip_address = None
        res = {'id': vip['id'],
        ...
        with context.session.begin(subtransactions=True):
            ...
            if v:
                try:
                    ...
                    raise loadbalancer.VipExists(pool_id=v['pool_id'])
                #vip_port_id = vip_db['port_id']
                #vip_port_details = core_db.get_port(context, vip_port_id)
                #vip_db.update({'subnet_id': vip_port_details['subnet_id'], 'ip_address': vip_port_details['ip_address']})
        return self._make_vip_dict(vip_db)
    ...
    def delete_vip(self, context, id):
        with context.session.begin(subtransactions=True):
            ...
            if vip.port:  # this is a Crd port
    ...
    def get_vip(self, context, id, fields=None):
        ...
        #vip_port_details = core_db.get_port(context, vip_port_id)
        #vip.update({'subnet_id': vip_port_details['subnet_id'], 'ip_address': vip_port_details['ip_address']})
    ...
    def _make_pool_dict(self, pool, fields=None):
        res = {'id': pool['id'],
               ...
               'provider': '',
	       'config_handle_id': pool['config_handle_id']
               }
    ...
    def create_config(self, context, config):
        ...
        pools = self.get_pools(context, filters=filters)
        res = {}
        if pools:
            pool_id = pools[0]['id']
            
            if pool_id:
                logical_config = self.get_logical_device(context, pool_id)
                lb_str = hacfg.generate_config(logical_config)
            
            with open('/tmp/haproxy.txt', 'w+') as f:
                f.write(str(lb_str))
                f.close()
    ...
    def get_logical_device(self, context, pool_id=None):
        core_db = network_db.CrdNetworkDb()
        #plugin = super(LoadBalancerPluginDb,self).__init__()
        with context.session.begin(subtransactions=True):
            qry = context.session.query(Pool)
            qry = qry.filter_by(id=pool_id)
            pool = qry.one()
            retval = {}
            retval['pool'] = self._make_pool_dict(pool)

            if pool.vip:
                retval['vip'] = self._make_vip_dict(pool.vip)
                retval['vip']['port'] = (
                    core_db._make_port_dict(pool.vip.port)
                )
                ...
                #    )
                fixed_ip = {}
                fixed_ip['subnet'] = core_db.get_subnet(context, retval['vip']['port']['subnet_id'])
                
            retval['members'] = [
                self._make_member_dict(m)
                for m in pool.members if (
                    m.status in constants.ACTIVE_PENDING_STATUSES or
                    m.status == constants.INACTIVE)
            ]
            retval['healthmonitors'] = [
                self._make_health_monitor_dict(hm.healthmonitor)
                for hm in pool.monitors
                if hm.status in constants.ACTIVE_PENDING_STATUSES
            ]
            #retval['driver'] = (
            #    plugin.drivers[pool.provider.provider_name].device_driver)
            with open('/tmp/log_device.txt', 'w+') as f:
                f.write(str(retval))
                f.close()
