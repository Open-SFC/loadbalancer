...
class VipExists(qexception.CrdException):
...
class StateInvalid(qexception.CrdException):
...
class MemberExists(qexception.CrdException):
    message = _("Member with address %(address)s and port %(port)s "
                "already present in pool %(pool)s")
    
##TODO::Temporary FIX for CRD
class AgentNotFound(qexception.NotFound):
...
RESOURCE_ATTRIBUTE_MAP = {
    'vips': {
        ...
        'status_description': {'allow_post': False, 'allow_put': False,
                               'is_visible': True},
        'port': {'allow_post': False, 'allow_put': False,
                    'default': [],
                    'is_visible': True},
    },
    'pools': {
